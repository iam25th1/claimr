import { Clock, Star, Video, MessageSquare } from "lucide-react"
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {featuredJobs.map((job) => (
          <div
            key={job.id}
            className="group relative overflow-hidden rounded-xl border border-yellow-500/30 bg-white/[0.03] p-5 backdrop-blur-sm transition-all hover:border-yellow-500/50"
            style={{
              boxShadow: "0 0 30px rgba(234, 179, 8, 0.1)",
            }}
          >
            <div className="absolute right-3 top-3 flex items-center gap-1 rounded-full bg-yellow-500/20 px-2 py-1 text-xs font-medium text-yellow-400">
              <Star className="h-3 w-3 fill-yellow-400" />
              Featured
            </div>

            <div className="flex items-start gap-4">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-yellow-500/20 to-orange-500/20 text-lg font-bold text-yellow-400">
                {job.avatar}
              </div>
